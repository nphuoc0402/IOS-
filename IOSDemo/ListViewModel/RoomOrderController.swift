        ...
        do {
            ...
            for data in results {
                if (!(checkinDate <= data.value(forKey: "checkoutDate") as! Date && checkoutDate >= data.value(forKey: "checkinDate") as! Date)) {
                    results.removeAll(where: { $0.roomId == data.value(forKey: "roomId") as? String && $0.checkinDate == data.value(forKey: "checkinDate") as? Date && $0.checkoutDate == data.value(forKey: "checkoutDate") as? Date  })
                }
            }
